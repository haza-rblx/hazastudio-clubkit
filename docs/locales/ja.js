...
window.ClubKitLocale = {
  ...
  strings: {
    ...
    "home.latest.title": "最新: 2.5.3",
    "home.latest.body": "Musicはグローバルのみ — ゾーンモードを削除（サーバー全体で1セッション）。",
    "home.latest.read72": "2.5.3 のノートを読む",
    "home.latest.upgrade72": "アップグレード 2.5.3",
    ...
    "home.footer.brand": "Hazastudio Clubkit · docs · エンジン 2.5.3",
    ...
    "updates.version": "現在 2.11.0",
    "updates.v2110.blurb": "ロールのグラデーションと名前チップ、ロール単位の権限、チート対策ガード。",
    "updates.v2110.a1": "<strong>多色ロールグラデーション</strong> — 1ロールにつき最大5色: <code>roleColor = { stops = { \"daad18\", \"ffed2a\", \"fdff90\" } }</code>。チャット色は最初と最後の色から自動で決まります。",
    "updates.v2110.a2": "<strong>ロール名チップ</strong> — <code>specialRank = { text = \"THE MINISTER\", gradient = true, chip = true }</code> でネームタグ上のチップ列にロール名をピル表示。そのプレイヤーの <code>#N ROBUX</code> / キャッシュのチップは隠れ、ランク行にはロールの <code>chatTag</code> が出ます。",
    "updates.v2110.a3": "<strong>ロール単位の権限</strong> — ロールがカテゴリの権限を上書きできます: <code>privileges = { canGift = false, adminPanel = false }</code>。見た目は同じスタッフ表示でも、権限を個別に変えられます。",
    "updates.v2110.a4": "<strong>チート対策ガード</strong> — 不正オーディオ（「見知らぬゲストが全員に音を流す」攻撃）、実行時のスクリプト注入、飛行 / スピード / ノークリップ / テレポート、レーザーのような巨大アバターをサーバー側で検知・報告します。今回の移動判定は<strong>ログモード</strong>で出荷：警告と報告のみで、まだキックはしません — すぐ有効にするなら <code>Config.MovementGuard.ENFORCE = \"kick\"</code>。",
    "updates.v2110.a5": "<strong>独自ローディング画面</strong> — キットが起動進捗をプレイヤーに公開し、あなたの画面（<code>ClubKitExternalLoading</code>）を待ちます。表示中はキットの音楽がミュートされます。",
    "updates.v2110.f1": "<strong>重いプレイスが起動しない</strong> — 4〜5 FPS の大規模会場でクライアントが30秒で諦め、キットUIが何も出ませんでした。現在は120秒待ちます。",
    "updates.v2110.f2": "<strong>Admin Hub の「Set role」</strong> — 選択肢が固定リストだったため、名前を変えたロールや追加ロールは <code>/setrole</code> でしか付与できませんでした。今は自分のロールカタログを読み込みます。",
    ...
    "updates.whatsLead": "プレイスで体感できる変更の要約。詳細は項目を開いてください。",
        "updates.v253.blurb": "Musicはグローバルのみ — ゾーンモードを削除。",
    "updates.v253.r1": "<strong>ゾーン音楽</strong> — サーバー全体で1セッション。<code>Music.MODE</code>、<code>ZoneTrackerService</code>、<code>MusicZoneChanged</code>、クライアントのゾーン処理を削除。Buyer <code>ClubKitConfig</code> は影響なし。",
    "updates.cat.removed": "Removed",
        "updates.v252.blurb": "Gravity、ライセンス/寄付、カップル、MusicCatalog、Discord、Music Library初回、モバイルAdmin Hub。",
    "updates.v252.f1": "<strong>Gravity</strong> — ダイヤル連動のidle-down落下＋ソフト着地。",
    "updates.v252.f2": "<strong>License</strong> — verifyタイムアウトで寄付ポーリングを止めない（fail-open）。",
    "updates.v252.f3": "<strong>Couple</strong> — 解消時にチャットタグをクリア；承認時にパートナータイトル表示。",
    "updates.v252.f4": "<strong>Music Library</strong> — 初回オープンでStudioプレースホルダに張り付かない。",
    "updates.v252.c1": "<strong>MusicCatalog</strong> — <code>playlists = { { name, tracks } }</code> を推奨（flatも可）。",
    "updates.v252.c2": "<strong>Top Menu Discord</strong> — <code>Branding.DiscordInvite</code>。空ならチップ非表示。",
    "updates.v252.c3": "<strong>Mobile</strong> — Admin Hub / JoinCommun のUIScale。",
...
"updates.v75.blurb": "ホットフィックス: DonationController の構文エラーでサーバー起動がブロック。",
    ...
    "updates.footer.brand": "Hazastudio Clubkit · Updates · エンジン 2.5.3",
