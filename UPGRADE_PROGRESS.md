...
| Release readiness (checked 2026-08-30) | Engine work since 2.10.0 is **release-ready**. ADR 0008 decision 4 settled 2026-08-30: `MovementGuard.ENFORCE = "log"` ships first, **flip to `"kick"` after one week of clean beacons (review due 2026-09-06)**. Verified this pass on the-basic (2 clients): role chip renders 123×31.5 px, label fits, 4-stop gradient exact, `RobuxRankWrapper`/`RupiahRankWrapper` hidden, special-rank row = chatTag; `SoundService.ClubKitMusic` group at volume 1 with music audible (loudness 65) when no pack is loading. Temporary Guest chip + `LoadingScreen` edits in that place were reverted. |
