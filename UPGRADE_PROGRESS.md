...
**Current version:** `2.6.6` (see [`VERSION`](VERSION))
...
- **Emote asset health visibility** — place-side dead `AnimationId`s (content-hash IDs that fail to resolve) produced "dance panel does nothing" symptoms and were diagnosed only via Studio logs. Candidate: startup summary from `AssetHealthReporter` ("N of M emotes failed to load") so content issues are distinguishable from engine bugs in seconds.
