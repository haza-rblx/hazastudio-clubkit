...
| Release readiness (checked 2026-08-30) | Engine work since 2.10.0 is release-ready **except ADR 0008 phase 1**: `MovementGuard.ENFORCE` still ships `"kick"` and the rules have never run on a live venue (custom teleport pads / elevators / vehicles untested) — owner decision 4. Verified this pass on the-basic (2 clients): role chip renders 123×31.5 px, label fits, 4-stop gradient exact, `RobuxRankWrapper`/`RupiahRankWrapper` hidden, special-rank row = chatTag; `SoundService.ClubKitMusic` group at volume 1 with music audible (loudness 65) when no pack is loading. Temporary Guest chip + `LoadingScreen` edits in that place were reverted. |
...
| `Shared/Config/RoleCategoryBuilder.luau`, `Shared/Config/ConfigBootstrap.luau`, `Shared/Domain/PermissionDomain.luau` | Per-role `privileges` override inside a category (`ROLE_PRIVILEGES`). |
| `Shared/Config/Roles.luau`, `Shared/Domain/Types.luau`, `Shared/Domain/OverheadDomain.luau`, `Server/Controllers/OverheadController.luau`, `Client/UI/OverheadUI.luau` | `specialRank.chip` → payload `roleChip` → `RoleChipWrapper` pill in `00-DonationLayers` with stops gradient; rank chips hidden for that role. |
...
| `extras/place-packs/CinematicLoading/` | **New place pack (not engine):** Hierapolis cinematic loading screen ported onto the external loading contract. Bridge now gates on the merged `LoadingScreen` (honours `HierapolisCustom` overlay). |
| `Shared/Config/ClubKitConfigSchema.luau`, `Hazastudio_ClubKitConfig/ClubKitConfig.luau`, `Shared/Config/ConfigBootstrap.luau`, `tools/ClubKitPackagerPlugin/plugin/PackagerCore.luau` | **New schema key `Features.HierapolisCustom`** (manifest group "More", default false). `ConfigBootstrap.applyHierapolisCustom` merges `Hazastudio_ClubKitConfig.HierapolisCustom` over the buyer state when true (warn + stock when the module is missing). Packager strips the module from packages + resets the flag in the blank template. |
| `extras/place-packs/Hierapolis/` | **New place pack (not engine):** `HierapolisCustom.luau` overlay module (President/Archon/DPC catalog, aliases, LoadingScreen off), `install.luau` (one-time: module + tool folders + CinematicLoading), `HierapolisToolFolders.rbxm`, README. Toggle = the flag above. |
